...
# heavy-tailed
def find_lognormal(L, U, bulk=0.90, precision=4):
    """
...
# still deciding whether or not to use Lppf, Uppf, or bulk
...
def find_cauchy(L, U, bulk=0.90, precision=4):
    """
    Cauchy Distribution
    ---------------------
    Pass in lower and upper values & ppf's (default 99%)
...
def find_studentt(ν, L, U, bulk=0.95, precision=4):
    """
    ...
    sigma_ = np.linspace(0.1, 100, 1000)
...
def find_gumbel(L, U, Lppf=0.005, Uppf=0.995, bulk=None, precision=4):
    """
    Gumbel Distribution
    ----------------------
    Pass in lower and upper values & ppf's (default 99%)
